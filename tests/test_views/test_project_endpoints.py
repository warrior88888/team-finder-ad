...
def test_toggle_favorite_requires_login(client, project):
    url = reverse("projects:toggle_favorite", kwargs={"project_id": project.pk})
...
def test_toggle_participate_requires_login(client, project):
    url = reverse("projects:toggle_participate", kwargs={"project_id": project.pk})
    response = client.post(url)
    assert response.status_code == 302
...
def test_complete_project_requires_login(client, project):
    url = reverse("projects:complete_project", kwargs={"project_id": project.pk})
    response = client.post(url)
    assert response.status_code == 302
