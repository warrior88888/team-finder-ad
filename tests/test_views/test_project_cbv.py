...
def test_project_detail_returns_404_for_missing(client):
    url = reverse("projects:project_detail", kwargs={"project_id": 99999})
    response = client.get(url)
...
def test_project_update_requires_login(client, project):
    url = reverse("projects:edit_project", kwargs={"project_id": project.pk})
    response = client.get(url)
    assert response.status_code == 302


def test_project_update_returns_404_for_missing(auth_client):
    url = reverse("projects:edit_project", kwargs={"project_id": 99999})
    response = auth_client.get(url)
    assert response.status_code == 404
...
def test_project_update_redirects_non_owner(auth_client, project_factory):
    ...
    expected_url = reverse("projects:project_detail", kwargs={"project_id": project.pk})
